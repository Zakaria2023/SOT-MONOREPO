...
describe("evaluateRule — project variable operands", () => {
  // Expected concurrent calls is a project decision, not a product spec: no
  // product carries it, so it can only come from the design's own answers.
  const pbxRule: EngineRule = {
    uuid: "var-1",
    name: "PBX concurrent calls",
    description: null,
    kind: "sum_budget",
    comparator: "lte",
    headroomPercent: 100,
    ...
    severity: "block",
    consumerSpec: {
      key: "expected-concurrent-calls",
      label: "Expected concurrent calls",
      unit: "calls",
      isVariable: true,
    },
    providerSpec: {
      key: "max-concurrent-calls",
      label: "Max concurrent calls",
      unit: "calls",
    },
  };

  const pbx = (capacity: string, quantity = 1): EngineItem => ({
    productUuid: "pbx",
    name: "PBX",
    quantity,
    attributes: { "max-concurrent-calls": capacity },
  });

  it("passes when the PBX covers the expected call volume", () => {
    const result = evaluateRule(pbxRule, [pbx("100")], [], {
      "expected-concurrent-calls": 60,
    });
    expect(result.status).toBe("pass");
    expect(result.demand).toBe(60);
  });

  it("fails when the expected volume exceeds capacity", () => {
    const result = evaluateRule(pbxRule, [pbx("30")], [], {
      "expected-concurrent-calls": 60,
    });
    expect(result.status).toBe("fail");
  });

  it("counts the variable once, however many phones are in the cart", () => {
    // The answer to "how many concurrent calls" doesn't multiply by quantity.
    const result = evaluateRule(pbxRule, [pbx("100", 3)], [], {
      "expected-concurrent-calls": 60,
    });
    expect(result.demand).toBe(60);
  });

  it("does not apply when the design hasn't answered the question", () => {
    // Treating unanswered as zero would silently pass every such rule.
    const result = evaluateRule(pbxRule, [pbx("30")], [], {});
    expect(result.status).toBe("not_applicable");
    expect(result.message).toContain("hasn't answered");
  });
});
