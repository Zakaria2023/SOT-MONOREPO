...
// A VENDOR-AUTHORED PAIR — this exact product works (or does not) with that one.
//
...
// existing rule the moment its values are filled in and a second vendor with the
...
//
// The `source` column is what keeps that honest. Rows imported from a vendor
...
export const ProductCompatibility = mysqlTable(
  "ProductCompatibility",
  {
    ...
    // `compatible` records a pair the derived rules would REFUSE and the vendor
    // allows. `incompatible` records the reverse — a pair the rules would pass
    // and the vendor says does not work.
    //
    ...
    // a person's name. Not decoration: a vendor matrix is dated and regenerable,
