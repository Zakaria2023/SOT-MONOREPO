  ...
  return (
    <div className="flex flex-col gap-3 rounded-control border border-hairline p-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs text-muted">Keyed by</span>
        {inputs.map((key) => (
          <span
            key={key}
            className="flex items-center gap-1 rounded-md bg-primary-tint px-2 py-1 text-xs font-medium text-primary"
          >
            {labelFor(key)}
            <button
              type="button"
            ...
            <thead>
              <tr className="border-b border-hairline text-left">
                {inputs.map((key) => (
                  <th
                    key={key}
                    className="px-2 py-2 text-xs font-semibold tracking-wide text-muted uppercase"
                  >
                    {labelFor(key)}
                  </th>
                ))}
                <th className="px-2 py-2 text-xs font-semibold tracking-wide text-muted uppercase">
      ...
      <button
        ...
        className="flex w-fit items-center gap-1.5 rounded-control border border-hairline px-3 py-2 text-xs font-semibold text-ink transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
      >
        <Plus size={14} />
        Add row
      </button>
